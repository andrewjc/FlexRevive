...
* Added shaders/common.hlsli and shaders/sweep.hlsl, the world collision pass: integrate, sweep the travelled segment against every collision mesh through its own grid, and land the piece on the first surface crossed with the normal, friction and spin response. This is the pass the frame time goes into, measured at 0.322 ms of a 0.565 ms frame, and the one that suits a card best because pieces are independent of each other for its whole duration
* Added shaders/pairs.hlsl, piece against piece, dispatched one colour of the cell partition at a time. It reuses the CPU pass's colouring, where two cells of one colour differ by three on some axis so the blocks they reach into cannot overlap, and keeps a cell's pieces on a single thread because colouring separates cells rather than the pieces inside one
* Added shaders/settle.hlsl and shaders/carry.hlsl: parking a piece the pile holds up, which no world contact ever reaches, and carrying each moving piece's particles by exactly the distance it turned out to travel
* Added the residency and dispatch layer: world geometry uploaded once and kept on the card, piece state uploaded per frame, every substep dispatched with nothing read between them, and a single readback at the end. That ordering is the whole design, since a readback measures 0.6 ms against 0.001 ms for a dispatch and any split that returns mid-substep pays it several times a frame
* Added the world pass comparison to tests/test_gpu.cpp: two thousand pieces dropped onto a triangle mesh through both paths, agreeing to 7.9e-06 on the worst component over twenty-four substeps, with nothing falling through the floor on either side. It caught a fault on the first run, where an empty upload copied from the null pointer an empty vector yields, which is the ordinary case for a mesh with no triangle index or a scene with no hulls
* Added src/gpu/Scene, which flattens the solver's containers into the arrays a shader indexes, and tests/test_gpuscene.cpp covering it without a graphics device. Marshalling is where a port of this kind actually goes wrong, and it goes wrong as debris in the wrong place rather than as anything that fails to build
* Added a GPU-side spatial hash rather than uploading the CPU one, which links with signed ints and -1 as its terminator: read as unsigned on a card that walks off the end of the world instead of stopping. The test walks every chain to prove it ends
* Changed every struct shared with a shader to be built from float4 and uint4 alone. A structured buffer packs tightly while a constant buffer pushes a straddling vector to the next register, so one declaration meant two layouts depending on where it was bound; a struct of float4s is identical under both rules and matches float[4] in C++ with nothing left to reason about. Verified against fxc /Fc rather than worked out by hand
...
* Added the pair, settle and world passes to the dispatch chain, so a whole frame runs on the card with nothing read back between substeps. The pair pass is dispatched one colour of the cell partition at a time, and two identical runs of a fifteen-hundred piece heap agree component for component, which is what says the colouring holds on a card as well as it does across threads
* Changed nothing about how debris is stepped, and the reason is now measured rather than estimated. A whole frame of a thousand pieces over two substeps costs 0.185 ms of dispatch and 8.2 ms of readback on an RTX 4090, against 0.945 ms for the same scene on the CPU path in game. The compute is genuinely fast; the synchronous round trip is not, and it is a fixed cost: twenty-four substeps of two thousand pieces read back in the same 8 ms as two substeps of one thousand. So ComputeBackend=gpu brings the device up, reports what it found, and steps on the CPU, which the log says plainly rather than claiming a GPU step it is not doing
...
* Fixed ForceEnableWeaponDebris doing nothing at all, which is why the mod only ever worked for people who had already set bNVFlexEnable=1 themselves. The game parses its ini files after F4SE has loaded its plugins, so every Setting object still holds its compiled-in default while a plugin is loading: the old code read bNVFlexEnable, always saw off whatever the user had configured, wrote on, logged that it had enabled weapon debris, and was then overwritten by the parse that followed. The line appeared in every session and had never meant anything
* Changed ForceEnableWeaponDebris to read and write Fallout4Prefs.ini rather than the live Setting, since while a plugin loads the file is the only account of the user's intent that exists. When debris is off it is enabled for the next launch and the log says plainly that the game must be restarted once, rather than claiming debris was turned on in a session where it cannot exist
* Added f4kit::engine::ReadGameIni and WriteGameIni. The write is read back rather than trusted, since a prefs file redirected by a mod manager can accept a write and keep the old contents
* Removed f4kit::archive::Mount, added earlier in this release to mount Fallout4 - Nvflex.ba2 when the setting had been turned on. It worked, and it was beside the point: the archive is skipped for the same reason the simulation is, so the only session it ran in was one where nothing could spawn anyway. The prefs fix covers both, and the mount is in the history if a use for it appears
...
* Fixed explosions doing nothing to settled debris, which is the whole point of a force field. The engine publishes a blast for exactly one update and states a strength of 100; integrated as an acceleration over that single frame it changed a chunk's speed by 18 units/s against the 25 the solver calls stationary, so a grenade landing in a pile of rubble moved none of it. It also made the result depend on frame rate, since multiplying by the substep gave a player at 30 fps twice the shove of one at 60
* Changed a blast to be applied once per frame as a direct change in velocity, outside the substep loop, exactly as an impact shock already was and for the same reason. Heavier chunks move less, and the engine's own strength still sets one explosion against another, so a Molotov stays feeble and a mini nuke does not
* Added BlastScale, since the engine's own figure is far too small to move anything at this game's scale. At 6.0 a chunk at the centre of a frag grenade leaves at about 600 units/s, falling off to nothing at the rim
* Fixed the settle pass zeroing the velocity of a piece a blast was still pushing. A supported chunk moving slower than the sleep threshold was parked and its velocity discarded, every frame, so an explosion could never accumulate into motion. A body under an external force is not at rest however slowly it happens to be moving
* Added the duration of a blast and the speed it actually produces to the log. Three diagnoses of this were wrong because they reasoned from the numbers rather than reading them; the one-line counter that measured how long a field is published settled it immediately
